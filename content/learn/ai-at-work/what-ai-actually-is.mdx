...
![How an LLM produces text: it breaks your prompt into tokens, predicts the most likely next token, and repeats until the reply is done](</diagrams/what-ai-actually-is/1-llm-next-token-prediction.png>)
...
![Model vs automation: automation follows pre-written rules and fails mechanically, while a model answers unpredictable situations and can be confidently wrong](</diagrams/what-ai-actually-is/2-model-vs-automation.png>)
...
![The word pile-up sorted: AI nests machine learning, which nests generative AI, which nests the LLM, with prompt, token, and agent as related terms](</diagrams/what-ai-actually-is/3-word-pile-up-hierarchy.png>)
