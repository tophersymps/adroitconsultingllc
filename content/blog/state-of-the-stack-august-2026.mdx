...
**Why it matters for clients:** if you're running MCP servers (and given Summer '26's Tableau MCP and Marketing Cloud MCP, more Salesforce shops are), you now have a version-compatibility job on your hands. The stateless redesign is the right call for scale, but it's a breaking change you should plan for rather than discover during an outage, and our [deep-dive on the stateless MCP revision](/field-notes/mcp-stateless-2026-07-28-production-agents) walks through the migration. This pairs with the GitHub Actions security roadmap we covered in depth this week: both are the same trend, the plumbing agents depend on is being hardened for production.
