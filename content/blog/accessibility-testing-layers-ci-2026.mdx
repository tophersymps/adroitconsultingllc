...
The layer that most teams skip entirely is a keyboard walk. This is not an automated scan; it is a person tabbing through every page in a real browser and answering a handful of questions. Is focus visible at every step? Does it move in a logical order? Is it ever trapped inside an open dialog? Does a skip link get the user past the navigation? The gap between automated tools and this walk is the class of problem a rule engine cannot model[^6]. axe can confirm a control is focusable, but it cannot tell you whether Tab moves in an order that matches the visual flow, whether the focus indicator is visible, whether a modal traps the keyboard, or whether a control is reachable by keyboard at all. Those are behavioral checks, and you have to drive them in a real browser, which is exactly what this walk does. Automation misses them, not because axe is weak but because keyboard operability and focus behavior are the kinds of things a rule engine cannot fully model.
...
[^6]: Kevin (dev.to), "Automated Accessibility Testing: axe-core, Keyboard Navigation and WCAG in the Browser." axe cannot tell you whether tab order makes sense, whether focus is visible, or whether a control is reachable by keyboard; those are behavioral checks you have to drive in a real browser. [dev.to](https://dev.to/kevinccbsg/automated-accessibility-testing-axe-core-keyboard-navigation-and-wcag-in-the-browser-2eib)
