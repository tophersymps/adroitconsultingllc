...
Most real-time app advice stops at the pipe. Pick Server-Sent Events when you only need server to client, reach for WebSockets when clients talk to each other, choose Supabase Realtime when you want Postgres to push the change for you. We have covered that transport layer in detail, both in [when SSE beats WebSockets](/field-notes/sse-vs-websockets-2026) and in [what changed when Supabase Realtime started shipping binary payloads](/field-notes/supabase-realtime-binary-payloads). The read side of a live app is genuinely solved: subscribe, get the event, update the screen.
