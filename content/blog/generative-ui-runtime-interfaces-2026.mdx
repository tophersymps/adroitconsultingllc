...
- Open, transparent components. Component libraries that require wrapping, style overrides, or mismatched APIs add abstraction layers the model cannot predict through. Every layer makes the output less consistent, which is why [headless UI component libraries](/field-notes/headless-ui-component-libraries-2026), which separate structure from styling, are the closest existing pattern to something a model can consume cleanly.
