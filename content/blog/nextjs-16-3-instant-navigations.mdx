...
This does not need to ship today (it is experimental), but it signals the direction: React compilation is moving from JavaScript to native code, the same path [TypeScript took with its Go-native compiler in v7.0](/field-notes/typescript-7-go-native-compiler).
