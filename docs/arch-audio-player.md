...
- `src/data/audio.ts` (generated by `scripts/build-audio.js`) — `import { ArticleAudio } from "@/lib/audio/contracts"`; `export const articleAudio: ArticleAudio[]`. Build-time static import, safe to import server-side. No client component imports `@/data/audio` today, and importing it client-side would ship bucket-relative object *keys* (non-secret, private bucket) into the JS bundle — so do not import it from a client component; the module was not designed to cross the client boundary.
...
| `Range` unsatisfiable (start beyond the end of the object) | 416 | no body, `Content-Range: bytes */<total>`, plus the same shared `BASE_HEADERS` as every other servable response (`Content-Type: audio/mpeg`, `Cache-Control: private, max-age=3600`, `Accept-Ranges: bytes`) |
