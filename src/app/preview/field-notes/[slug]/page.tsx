...
/**
 * /preview/field-notes/[slug] — auth-gated request-time render of a draft MDX.
 *
 * Reads content/blog/[slug].mdx directly via the existing MDX pipeline
 * (the public /field-notes/[slug] page is static and excludes drafts by
...
export default async function PreviewBlogPage({ params }: Props) {
    ...
    return (
      <div className="min-h-screen flex flex-col">
        ...
        <main id="main" className="flex-1">
          <DraftLocked
            state="signed-out"
            nextPath={`/preview/field-notes/${slug}`}
          />
        </main>
        <Footer />
      </div>
  ...
  if (!isPreviewEmailAllowed(user.email)) {
    return (
      <div className="min-h-screen flex flex-col">
        ...
        <Header />
        <main id="main" className="flex-1">
          <DraftLocked state="no-access" nextPath={`/preview/field-notes/${slug}`} />
        </main>
        <Footer />
      </div>
    );
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <ReadingProgress />
      <Header />
      <PreviewStrip title={title} status={status} backHref="/field-notes" />

      <main id="main" className="flex-1">
        {/* Post Hero (matches public /field-notes/[slug], minus ShareBar/progress) */}
