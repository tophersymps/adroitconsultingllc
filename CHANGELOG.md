...
### Configuration Walkthrough audio bridge: keep + reframe (t_628c416b)

**What** - Lesson narration now BRIDGES AND KEEPS the Configuration Walkthrough section (ADR-106/107 keep + bridge), instead of reading it verbatim as raw mouse-click steps. `mdxToNarration` lesson mode detects walkthrough-style headings (anchored on the normalized text: `Configuration Walkthrough`, `Configuration steps`, `Setup walkthrough`, bare `Walkthrough`, plus `config`/`setup` prefixes) and prepends a single spoken framing line before the section: "Here is how this is configured in a sandbox. I will walk through the steps and the reasoning. You can run the exact clicks when you are back at the UI." The section body (sequence + why + exam traps) is then read as normal — it is NOT cut, unlike `Try It`. The bridge is emitted once per lesson. New `walkthroughBridge` option on `NarrationOverrides` / `NarrationOptions` (defaults to the line above; em-dash-free). Article mode (`lesson` falsy) is byte-for-byte unchanged.

**Why** - Chris approved a keep + bridge approach (2026-09-18). The Configuration Walkthrough is where the mechanism and exam traps live, so it must not be cut; but it is written for a reader at the UI, so the clicks are un-actionable to a listener. The bridge reframes the section for audio while preserving the learning content.

**Verified** - `npx vitest run src/lib/audio-narration.test.ts src/lib/audio-emit.test.ts` (44 tests, incl. 5 new walkthrough cases); `npx tsc --noEmit` exits 0; `npm run build` and `npm run lint` pass. Ran `mdxToNarration(raw, {lesson:true})` on the real lesson `day-01-p-1a-object-permissions-crud-system-vs-object-profiles.mdx`: the Configuration Walkthrough now opens with the bridge line then reads the walkthrough content (steps 1 and 8 verified), Try It still cuts + bridges, Related Requirements + References still skipped, diagrams still narrated, ends with the knowledge-check transition, zero em-dashes in new prose.

**Known Issues** - None. Committed to `feat/lesson-audio-walkthrough-bridge` only; main is untouched.

### Lesson audio refinements: smart lesson narration + TTS timeout scaling (t_36fb4f74)

**What** - Two refinements to lesson audio on `feat/lesson-audio-refinements` (NOT merged to main). (1) `mdxToNarration` gains an opt-in `lesson` mode (ADR-106/107): section routing is gated on an interactive-heading ALLOWLIST, so a lesson reads only its LEARNING content - Deep Dive, Worked example, Configuration Walkthrough, Exam Traps, What's Next (recap) and any concept subheads - and cuts the hands-on `Try It` step-by-step (replaced by a single spoken bridge line, once) plus `Related Requirements` and `References` (nothing emitted). The knowledge-check transition is appended once as the final line, only when learning content was actually emitted. Article mode (`lesson` falsy) is byte-for-byte unchanged: the blog path and the 52 pre-refinement audio tests do not regress. Diagram narration (`Diagram: <alt>.`), subheading cues, and syntax stripping are preserved. (2) `scripts/build-audio.js` replaces the rigid `timeout: 120000` with a length-scaled `ttsTimeoutMs(narration)` = `min(30s + 90ms/word, 15min)` (ADR-108), applied to BOTH the blog and learn synthesizers, so a ~4k-word lesson (previously killed at exactly 120s) gets ~6.6 min and the max 5,713-word lesson ~9 min. New `NarrationOptions` fields mirrored in `src/lib/audio/contracts.ts`.

**Why** - Chris's requirement: a listener cannot act on step-by-step instructions, so reading them aloud is noise, and every lesson should end with a hand-off to its real, separate knowledge-check surface. And the generator's rigid 120s Kokoro timeout killed long lessons at exactly 120s.

**Verified** - Full narration + audio + atlas suite passes (`npx vitest run src/lib/audio-narration.test.ts src/lib/audio-emit.test.ts audio/* atlas/*`: 115 tests, incl. 12 new lesson-mode cases); `npx tsc --noEmit` exits 0; `npm run build` and `npm run lint` pass. Ran `mdxToNarration(raw, {lesson:true})` on the real 5-diagram lesson `day-01-p-1a-object-permissions-crud-system-vs-object-profiles.mdx`: 4,084 words, learning content (Deep Dive + subheads, Worked example:, Configuration Walkthrough, Exam Traps, What's Next) read, all 5 `Diagram:` descriptions read, Try It step-by-step NOT read (one bridge line), Related Requirements + References NOT read, ends with the KC transition, zero leaked syntax, zero em-dashes in new prose. `build-audio.js --metadata-only --learn` emits all 240 lessons with `lesson:true` (EXIT 0). ttsTimeoutMs computes 398s (4,084w), 544s (5,713w), cap 900s.

**Known Issues** - None. Committed to `feat/lesson-audio-refinements` (worktree branch `feat/lesson-audio-refinements-t_36fb4f74`, based on design commit cec175b) only; main is untouched. The `--metadata-only --learn` run's regenerated `src/data/lesson-audio.ts` was reverted to avoid clobbering the (still-empty scaffolding) generated file; backfill runs later.
...
**Why** - A single production build used to spawn 11 concurrent build workers at roughly 150 MB RSS each. That ~2.5–3 GB transient lands on a machine where two local LLM servers pin ~36.9 GB (llama-server Qwen3.8-27B on :8080, gemma-4-12b on :8081), and the overlap pushed the kernel into swap — `/System/Volumes/VM/swapfile0` was written during the 2026-09-16 deploy-gate build exactly when the build ran.

**Verified** -
- `npx tsc --noEmit` -> exit 0.
- `npm run build` -> exit 0, three consecutive runs (two warm-cache, one cold with `.next/` moved aside). Next's own output now reports the capped count in **both** worker phases: `Collecting page data using 4 workers ...` and `Generating static pages using 4 workers (535/535) in 3.1s`; before the change those lines report the default 11.
- Process-tree sampling every 400 ms for the whole build (descendants of the build command, so the shell, the `next-build` coordinator, the `jest-worker` children and the `turbopack-node` pools are all counted): warm-cache peak **7 processes / 1.69 GB tree RSS** (coordinator 0.56 GB + 4 `jest-worker` children 0.16-0.31 GB + 1 postcss transform pool 0.14 GB); cold-cache peak **8 processes / 2.88 GB tree RSS** (`next-build` coordinator 1.69 GB + 4 `jest-worker` children 0.17-0.33 GB + 2 postcss transform pools 0.12/0.06 GB). Excluding the coordinator, worker RSS peaks at **1.04 GB warm / 1.10 GB cold**; the 4 capped `jest-worker` children alone total 0.90-0.95 GB, against ~1.65 GB for the 11 the default produced.
- Negative control, same machine and repo: the identical `npm run build` run on `main` (**without** this change) reports `Collecting page data using 11 workers ...` / `Generating static pages using 11 workers (543/543)`. With the change, both phases report 4.
- No new swap: `vm_stat` `Swapouts` read **3972 before and 3972 after** the monitored build, and were still 3972 after every later build (cold and control runs included).

**Known Issues** - The cap does what the card asked (worker fan-out 11 -> 4; the four `jest-worker` children total 0.90-0.95 GB instead of ~1.65 GB), but it does **not** deliver the card's "~0.8 GB build spike" estimate and should not be assumed to have removed the swap pressure on its own: the largest single term in a cold build is the one `next-build` coordinator process, which grows to **1.69 GB RSS** and is not governed by `experimental.cpus`. Measured total peak is still ~2.9 GB on a cold build (~1.7 GB warm-cache). Further reduction needs either a memory cap on the coordinator (`NODE_OPTIONS=--max-old-space-size`) or the build moving off the box while the pinned models are resident. Note also that neither the capped nor the uncapped build wrote to swap during this session, so the reduction was not re-verified against the original 2026-09-16 pressure conditions. Two `content/learn/**/questions/*.json` files are regenerated with a slightly different slug by `npm run prebuild` (tracked as deleted + new untracked); that is pre-existing generator behaviour, unrelated to this change, and was left out of the commit.

### Fix: Follow-along no longer un-checks itself when the browser's first smooth-scroll event arrives late (t_8ef99cf7)

**What** - The t_a34614a1 fix removed the 800 ms cap as a mid-animation trigger, but left a second race in `beginProgrammaticScroll` (`src/components/BlogPost/AudioPlayer.tsx`): the settle timer was armed with the **150 ms inter-event window at suppression start** — before the animation had emitted a single event. Chrome does not fire the first `scroll` event for a smooth document scroll synchronously with the `scrollTo` call; the a11y re-verification measured **158.8 / 184.7 / 205.3 / 306.6 ms** (and 244.2 ms for a bare probe in a visible, focused, unthrottled tab). So the timer expired first, the animation's first event landed on the **post-suppression** path, the position-based tail check could not match (nothing had ever been suppressed, so `lastSuppressedScrollAtRef` was still `0`), and the code fell through to `setFollowing(false)`. Same user-visible symptom, reproduced **4/6 live runs (2/2 at 1x CPU, no polling)** with 1.1–1.5 s bursts — neither cap was involved.

1. **Animation-start grace (`SCROLL_START_GRACE_MS = 1500`).** Until one `scroll` event of the current animation has been absorbed there is no inter-event cadence to measure, so the first timer must span the browser's scroll-start latency (~5x the worst measured value). The instant an event is absorbed the window switches back to the 150 ms no-scroll settle test. `lastSuppressedScrollAtRef` is reset per programmatic scroll so a later block change gets the grace too, and the 4000 ms cap remains a stall guard only.
2. **Late-first-event rescue.** If a `scroll` event arrives after suppression already ended (a nested scroller's `scrollend` bubbles to `window` and the listener cannot tell it apart from ours), and nothing has been suppressed yet, and the offset is still **on its way to** the target we asked `scrollTo` for (`|scrollY - target| > SCROLL_TAIL_EPSILON_PX`) within the start grace, it is still ours: suppression is re-opened for the remainder of the burst (bounded by the same stall cap) instead of un-checking the toggle.
3. **AC-4 is preserved.** A bare `scroll` (scrollbar drag — no wheel/touch/key) once the animation has settled still stops following, audio untouched; and the grace is bounded, so a programmatic scroll whose animation emitted nothing at all cannot swallow a reader's drag after it expires.

**Why** - lara's live re-verification of PR #244 (3cd7949, card t_a8266062) returned NEEDS_REVISION on the central promise: 4 of 6 real-browser bursts un-checked the toggle, because the 150 ms window was armed before the animation had emitted anything. The existing suite could not see it: every emulated burst there starts on the first frame.

**Verified** -
- Four new tests in `src/components/BlogPost/AudioPlayerScroll.test.tsx` (`stubScrollToWithSmoothBurst` gained a `firstDelayMs` option): a **late first event at 200 ms** with the rest on the 16 ms cadence, the **premature-`scrollend`** variant, plus two AC-4 guards (a bare scroll after a late-first burst has settled still un-checks; a bare scroll after the start grace still un-checks when the animation emitted nothing). On `3cd7949` (fix stashed) **3 of the 4 fail** (`toBeChecked()` reading an un-checked toggle / `seen.every(checked)` false) and all **4 pass** with the fix. The two AudioPlayer files: 27/27.
- Gates: `npx vitest run` -> 99 files / **818 tests pass**. `npx tsc --noEmit` -> exit 0. `npx eslint` -> clean. `npx next build` -> exit 0.
- Live, real Chrome on `next start`, signed in as the QA reader, on `/field-notes/accessibility-testing-layers-ci-2026`: with the animation's start deferred 200 ms so the first event lands **327 / 404 / 393 ms** after the `scrollTo`, the toggle stayed **checked at every event and at the end**, one `scrollTo` per block, no same-block re-scroll. The same three runs on the **pre-fix build** un-checked at **386 / 348 ms** (first events 358.7 / 247.1 ms) — the reported race, reproduced and fixed. Undeferred real bursts (first events 26–125 ms) also stay checked. AC-3 (wheel / ArrowDown stop following, audio untouched), AC-4 (bare scroll after settle un-checks, audio untouched) and the 93x44 hit target re-verified live on the fixed build.

**Known Issues** - None. The rescue is deliberately narrow: it needs *no* event ever suppressed, the position still short of our own target, and the start grace. A reader who drags the scrollbar during that window in the same direction as a pending auto-scroll could have that one drag absorbed — the next event with normal cadence ends the window.

### Fix: the Follow-along suppression cap is no longer shorter than a real smooth-scroll burst (t_a34614a1)

**What** - The t_078c9385 fix gated stop-following on real user *input* (wheel/touch/scroll-keys), which was correct, but it left one wall-clock timer in the path: the suppression window for OUR OWN programmatic scroll was **hard-capped at 800 ms** (`SCROLL_SUPPRESS_CAP_MS`, applied in `beginProgrammaticScroll`). Chrome's smooth-scroll event burst is **longer than that** — the a11y review measured 731 ms / 768 ms / 851 ms for an ordinary one-screen (~1000 px) jump and 1515 ms for a three-screen jump — so suppression expired *mid-animation*, the next `scroll` event from our own scroll hit the listener with the gate already open, and the toggle un-checked itself. The original symptom, moved from a 1 ms window to an 800 ms one. Three changes:

1. **The cap is now a stall guard, not a timing signal (800 ms -> 4000 ms).** `scrollend` (where supported) and the "no `scroll` event for 150 ms" settle window are the real end-of-suppression triggers; they self-terminate when the animation actually stops. The cap exists only so a stalled animation can never wedge following into a permanently suppressed state, so it now sits far above every measured burst instead of inside their range. A wheel/touch/scroll-key input still ends suppression instantly.
2. **The tail of our own animation is identified by position, not by a timer.** `handleTimeUpdate` records the `top` it asked `window.scrollTo` for; the `scroll` listener records when the last suppressed event arrived. A post-suppression `scroll` event that arrives on the animation's own cadence (`<= SCROLL_SETTLE_MS` since the last suppressed event) AND sits within `SCROLL_TAIL_EPSILON_PX` (2 px) of that target is still ours and is ignored. A signal that fires a frame early can no longer make our own scroll look like user input.
3. **AC-4 is preserved.** A bare `scroll` with no preceding input, after our animation has settled, still stops following (audio keeps playing). The tail tolerance only covers events still parked at the offset we asked for, on the animation's cadence — the reader's scrollbar drag lands elsewhere and after the window has closed.

LOW a11y finding folded in: the "Follow along" toggle's label grew from a 93x16 px target to `min-h-11` (44 px tall, `cursor-pointer`) so it meets the house 44x44 minimum instead of only barely satisfying WCAG 2.2 SC 2.5.8's spacing exception. Focus ring and keyboard operation are untouched.

**Folded in from sato's perf review of b62e75e (t_fdd0654c, P1 — LOW).** The alignment cache added by t_05dc3b79 was keyed on `slug` + the `timings` ref only, so an empty `articleBlocks()` result (article body not yet in the DOM on the first tick, or detached by a remount) was cached for the whole mount and Follow-along silently stopped working for that page view. The cache miss condition now also rebuilds when `cached.blocks.length === 0` or the first cached block element is no longer `isConnected` — the happy path is unchanged (one alignment build per article). New regression test `recovers when the article DOM only appears after the first tick (never caches an empty alignment)` **fails without the guard** and passes with it.

**Why** - The a11y review of PR #239 (t_af105c84) reproduced the original "Follow along unchecks itself on every auto-scroll" symptom live in a real browser on bdd88ca: suppression ended at ~855 ms while our own smooth scroll was still emitting events. The unit suite could not see it because the emulated burst was 6 events x 16 ms = **96 ms** — shorter than any plausible cap, so the cap value was untested. (Also fixed here: the emulated burst now advances `window.scrollY` frame by frame, as a real smooth scroll does.)

**Verified** -
- New tests in `src/components/BlogPost/AudioPlayerScroll.test.tsx`: a parametrized burst longer than the old cap (896 ms and 1600 ms, plus the existing 96 ms case) and a **large-jump** case (a multi-screen target whose 96 x 16 ms = 1536 ms burst matches the 1515 ms live measurement). `npx vitest run` on the two AudioPlayer files **fails 3 / passes 14 on the unfixed component** (all three failures are `toBeChecked()` reading the un-checked toggle) and **passes 22 / 22** after the fix. All AC-1/AC-3/AC-4/AC-5 tests stay green.
- Gates: `npx vitest run` -> 99 files / **813 tests pass**. `npx tsc --noEmit` -> exit 0. `npm run lint` -> clean. `npm run build` -> exit 0.
- Live-browser re-measurement of the repro (dev server + signed-in reader): see the completion evidence on the card — the checkbox stays **checked** through the entire recorded burst (both the one-screen jump and the multi-screen jump) with exactly one `scrollTo` per block change and no same-block re-scroll.

**Known Issues** - None. The tail tolerance is deliberately narrow (150 ms cadence + 2 px of the requested target); a reader who parks the scrollbar exactly on the auto-scrolled offset within 150 ms of the animation ending still sees the next movement un-check the toggle, which is the intended AC-4 behaviour.
...
1. **Gate on user input, not on `scroll`.** `wheel`, `touchmove`, and the scroll keys (Arrow/Page/Space/Home/End) are never generated by a programmatic scroll, so they are now the authoritative stop-following signal and need no suppression at all.
2. **A real suppression window for our own scroll.** `src/components/BlogPost/AudioPlayer.tsx` opens the suppression before `window.scrollTo` and closes it when the scroll *settles*: `scrollend` where supported (Chrome 114+/Safari 18.2+/Firefox 109+), else a "no scroll for 150ms" window extended by each in-flight event, hard-capped at 800ms so a stalled animation can never permanently suppress following. A wheel/touch/key input during the window ends suppression immediately.
3. **Scroll only when the active paragraph changes.** The 250ms same-block re-scroll is deleted; `lastScrollBlockRef` is now purely a change detector, so playback no longer keeps the page in a continuous animation inside one paragraph.
4. **Cleanup.** `following` dropped from the listener effect's dependency array (the handlers never read it), and the dead `onWheel`/`onTouchStart`/`onScroll` React props on the non-scrollable wrapper and `onScroll` on `<audio>` are removed (the window listeners already cover the same input).

The toggle default (ON signed-in, OFF under `prefers-reduced-motion`), the logged-out locked card, and the pinned/floating animation are unchanged.

**Why** - Reported by topherdev (Discord): "Follow along" un-checked itself after every paragraph, so no reader could use follow-along; the feature disabled itself on first use. Root cause confirmed by a repro test that emulates the real smooth-scroll event burst. The pre-existing `AudioPlayerScroll.test.tsx` was green on the broken code because its `window.scrollTo` mock is a bare `vi.fn()` that emits no scroll events — the bug was structurally invisible to it.

**Verified** - The repro (and the new AC-1/AC-3/AC-4 regression tests) **fail on the unfixed code** (5 failed) and **pass after** the fix, committed permanently to `src/components/BlogPost/AudioPlayerScroll.test.tsx`. `npx vitest run` → 99 files / **809 tests pass**. `npx tsc --noEmit` → exit 0. `npm run lint` → clean on the changed files.

**Known Issues** - A scrollbar drag is still detectable as a stop via the remaining `scroll` listener (it fires neither wheel nor touch), but only after our own animation has settled — a drag beginning *during* a follow-along smooth scroll is treated as part of the animation until it settles, which matches the previous intent.
