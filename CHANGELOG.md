...
### Security fix: --bitrate threaded through the shell-free argv path — build-audio.js rebased onto the deployed execFileSync fix (t_78e28287)

**What** - The lean-audio commit is now based on `origin/main` (`c23264d`, which already contains `070efa1` + `b3fa284`) instead of `021a77e`, and the `--bitrate` value travels as an **argv element** into the existing shell-free `execFileSync(venvPython, [engine, ...])` call — the `execSync`/`synthCmd` shell string and the temp `.narration.txt` file are gone for good. A new `BITRATE_RE = /^\d{2,3}k$/` allowlist (`assertSafeBitrate`, called once in `main()` alongside `assertSafeVoice`) rejects anything else before it reaches the engine; `scripts/reencode-audio-lean.cjs` validates `--bitrate` with the same regex.
...
**Verified** - val-el's PoC (`poc-t6aeb68f3-bitrate-injection.cjs`) re-run against the rebased emitter both ways: no marker file for `AUDIO_BITRATE='48k; touch … #'` or `--bitrate '48k; touch … #'`; the injectable value now aborts with `FATAL: invalid bitrate … rejected`. `git merge origin/main` into the rebased branch is a no-op (no conflict); `assertSafeSlug`/`assertSafeVoice`/`execFileSync` all present, zero `execSync` and zero `synthCmd` in `scripts/build-audio.js`. New bitrate regression tests in `src/lib/audio-emulation.injection.test.ts`; `npm test` 742/742 pass; `npm run build` exit 0; `npm run lint` clean.
