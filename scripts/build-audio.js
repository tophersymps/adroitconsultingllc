...
const LESSON_OUT_PATH = path.join(ROOT, "src", "data", "lesson-audio.ts");
...
/*
 * `series` is a DIRECTORY NAME from content/learn/<series>/. It is
 * interpolated into filesystem paths and storage keys (learn/<series>/<slug>/),
 * so it gets the same strict allowlist as a slug — a hostile series name must
 * never reach a path or a bucket key (CWE-22 path traversal).
 */
function assertSafeSeries(series) {
  if (typeof series !== "string" || !SLUG_RE.test(series)) {
    throw new Error(
      `invalid series ${JSON.stringify(series)} rejected: must match ${SLUG_RE} (content/learn directory name)`
    );
  }
}
...
/* --learn [series]: target lessons (content/learn/<series>/*.mdx) instead of
 * blog articles, emitting learn/<series>/<slug>/ keys into
 * src/data/lesson-audio.ts. Optional value scopes to one series. The value is
 * only consumed when the token after --learn is NOT another flag (so
 * `--learn --metadata-only` means "all series", not series "--metadata-only"). */
const learn = args.includes("--learn");
const learnIdx = args.indexOf("--learn");
const learnNext = learnIdx >= 0 ? args[learnIdx + 1] : undefined;
const learnSeriesOnly = learnNext && !learnNext.startsWith("--") ? learnNext : null;

if (metadataOnly && !recentN && !slugOnly && !backfill && !learn) {
  console.error("--metadata-only requires --recent N or --slug or --learn.");
  process.exit(2);
}
...
/* --- emit src/data/lesson-audio.ts (learn mode, ADR-101) --- */
function emitLesson(entries) {
  entries.sort((a, b) => {
    const bySeries = a.series.localeCompare(b.series);
    return bySeries !== 0 ? bySeries : a.slug.localeCompare(b.slug);
  });
  const lines = [
    "/**",
  ...
  for (const e of entries) {
    lines.push(
      `  { series: ${JSON.stringify(e.series)}, slug: ${JSON.stringify(e.slug)}, voice: ${JSON.stringify(e.voice)}, storagePath: ${JSON.stringify(e.storagePath)}${e.timingsStoragePath ? `, timingsStoragePath: ${JSON.stringify(e.timingsStoragePath)}` : ""} },`
    );
  }
  lines.push("];", "");
  fs.writeFileSync(LESSON_OUT_PATH, lines.join("\n"));
  console.log(`Wrote ${LESSON_OUT_PATH}: ${entries.length} entries`);
}
...
/* Parse an existing src/data/lesson-audio.ts into
 * {series,slug,voice,storagePath[,timingsStoragePath]}. Used by --learn
 * --backfill/--incremental to MERGE rather than replace, so previously
 * generated lessons (incl. any Tier C timingsStoragePath field) are preserved. */
function readExistingLessonEntries() {
  if (!fs.existsSync(LESSON_OUT_PATH)) return [];
  const raw = fs.readFileSync(LESSON_OUT_PATH, "utf-8");
  const out = [];
  const re = /\{\s*series:\s*"([^"]+)",\s*slug:\s*"([^"]+)",\s*voice:\s*"([^"]+)",\s*storagePath:\s*"([^"]+)"(?:,\s*timingsStoragePath:\s*"([^"]+)")?\s*\}/g;
  let m;
  while ((m = re.exec(raw))) {
    const e = { series: m[1], slug: m[2], voice: m[3], storagePath: m[4] };
    if (m[5]) e.timingsStoragePath = m[5];
    out.push(e);
  }
  return out;
}
...
/* --- learn mode (ADR-101/102): narrate lessons into src/data/lesson-audio.ts ---
 *
 * Mirrors the blog path but targets content/learn/<series>/*.mdx and emits
 * learn/<series>/<slug>/<voice>.mp3 keys into src/data/lesson-audio.ts. The
 * narration builder (mdxToNarration) is reused unchanged — it already handles
 * lesson MDX (verified on a 5-diagram lesson: 0 leaked syntax). The merge is
 * keyed on series/slug/voice so a run can never drop a lesson it did not
 * regenerate, exactly like the blog union.
 */
async function runLearn({ env, dualWrite, mdxToNarration }) {
  /* A --learn <series> value scopes to one series; bare --learn targets every
   * series directory. Both are validated before they reach a path or key. */
  let seriesList;
  if (learnSeriesOnly) {
    assertSafeSeries(learnSeriesOnly);
    seriesList = [learnSeriesOnly];
  } else {
    seriesList = fs
      .readdirSync(LEARN_DIR, { withFileTypes: true })
      .filter((d) => d.isDirectory())
      .map((d) => d.name);
    for (const s of seriesList) assertSafeSeries(s);
  }

  /* NEVER lose state: seed the union from whatever src/data/lesson-audio.ts
   * already holds on EVERY path, keyed by series/slug/voice. */
  const entries = readExistingLessonEntries();
  const byKey = new Map(entries.map((e) => [`${e.series}/${e.slug}/${e.voice}`, e]));
  const existingKey = new Set(byKey.keys());
  const limitN = limit ? parseInt(limit, 10) : null;
  let synthesized = 0;

  function putLessonEntry(entry) {
    const key = `${entry.series}/${entry.slug}/${entry.voice}`;
  ...
  for (const series of seriesList) {
    const seriesDir = path.join(LEARN_DIR, series);
    if (!fs.existsSync(seriesDir)) {
      console.error(`ERROR: series dir ${seriesDir} does not exist`);
      process.exit(1);
    }
    const slugs = fs
    ...
    for (const slug of slugs) {
      assertSafeSlug(slug);
      if (incremental && !force && existingKey.has(`${series}/${slug}/${voice}`)) {
        console.log(`SKIP ${series}/${slug}: ${voice} already in lesson-audio.ts`);
        continue;
      }
      if (limitN !== null && synthesized >= limitN) {
        console.log(`STOP ${series}/${slug}: reached --limit ${limitN}`);
        break;
      }
      ...
      if (metadataOnly) {
        putLessonEntry({ series, slug, voice, storagePath });
        continue;
      }

      // synthesize via the TTS engine CLI (same NO-SHELL argv discipline as the
      // blog path - every value a single argument, allowlisted before use).
      const out = path.join(ROOT, ".audio-out", `${series}--${slug}.mp3`);
      const timingPath = path.join(ROOT, ".audio-out", `${series}--${slug}.timing.json`);
      ...
      if (!/audio|mpeg|wave|WAVE/i.test(finfo)) {
        console.error(`ERROR ${series}/${slug}: produced file is not audio (${finfo || "unknown"})`);
        process.exit(1);
      }
      ...
      try {
        ...
        console.error(`ERROR ${series}/${slug}: upload of ${storagePath} failed (${String(e.message).slice(0, 400)})`);
        process.exit(1);
      }
      const size = fs.statSync(out).size;
      console.log(`OK ${series}/${slug} ${voice} ${bitrate} ${size} bytes -> ${storagePath}`);

      // Tier C timing manifest (same rule as the blog path: REQUIRED for a real
      // generation - abort rather than emit an entry whose Follow-along would
      // silently be wrong).
      const timingStoragePath = `learn/${series}/${slug}/${voice}.timing.json`;
      if (!fs.existsSync(timingPath)) {
        console.error(`ERROR ${series}/${slug}: engine did not emit ${timingPath} (--timing missing?)`);
        process.exit(1);
      }
      const timingsBuf = fs.readFileSync(timingPath);
      JSON.parse(timingsBuf.toString("utf-8"));
      try {
        await writeObject({ env, storagePath: timingStoragePath, buf: timingsBuf, dualWrite });
      } catch (e) {
        console.error(
          `ERROR ${series}/${slug}: upload of ${timingStoragePath} failed (${String(e.message).slice(0, 400)})`
        );
        process.exit(1);
      }
      console.log(`OK ${series}/${slug} timings ${timingsBuf.length} bytes -> ${timingStoragePath}`);

      putLessonEntry({ series, slug, voice, storagePath, timingsStoragePath: timingStoragePath });
      synthesized++;
    }
  }

  emitLesson([...byKey.values()]);
}
...
async function main() {
  ...
  /* Learn mode (ADR-101/102): target lessons instead of blog articles. The
   * blog path below is untouched; learn mode runs its own enumeration, merge
   * and emit into src/data/lesson-audio.ts. */
  if (learn) {
    await runLearn({ env, dualWrite, mdxToNarration });
    return;
  }
