...
// Allowlist: content/ + safe path chars, rejecting ".." path segments and any
// shell metacharacters. Prevents an injected filename from ever reaching a
// shell (CWE-78) — execFileSync passes argv without /bin/sh.
const relRe = /^content\/[A-Za-z0-9][A-Za-z0-9._/-]*$/;
for (const rel of changed) {
  if (!relRe.test(rel) || rel.includes("..")) {
    console.log("SKIP (non-allowlisted path): " + rel);
    continue;
  }
  const headText = execFileSync("git", ["show", `HEAD:${rel}`]).toString();
